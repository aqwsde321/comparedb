...
-- 읽기 전용 사용자 생성
CREATE USER readonly_user WITH PASSWORD 'read_pw_jjjj';

-- 읽기 전용 사용자에게 CONNECT 권한 부여
GRANT CONNECT ON DATABASE comparedb TO readonly_user;

-- 읽기 전용 사용자에게 USAGE 권한 부여
GRANT USAGE ON SCHEMA public TO readonly_user;

-- 읽기 전용 사용자에게 모든 테이블에 대한 SELECT 권한 부여
GRANT SELECT ON ALL TABLES IN SCHEMA public TO readonly_user;

-- 향후 생성될 테이블에 대해서도 자동으로 SELECT 권한 부여
