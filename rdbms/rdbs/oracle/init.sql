...
-- 사용자 생성
CREATE USER comparedb IDENTIFIED BY read_pw_jjjj;

-- 권한 부여
GRANT CONNECT, RESOURCE TO comparedb;
GRANT UNLIMITED TABLESPACE TO comparedb;

-- 테이블스페이스 쿼터 설정
ALTER USER comparedb QUOTA UNLIMITED ON users;

-- 스키마 전환
ALTER SESSION SET CURRENT_SCHEMA = comparedb;
...
-- comparedb_tables_read 역할 생성
CREATE ROLE comparedb_tables_read;

-- comparedb 스키마의 모든 테이블에 대한 SELECT 권한 부여
BEGIN
    FOR t IN (SELECT table_name FROM all_tables WHERE owner = 'COMPAREDB') LOOP
        EXECUTE IMMEDIATE 'GRANT SELECT ON comparedb.' || t.table_name || ' TO comparedb_tables_read';
    END LOOP;
END;
/

-- readonly_user에게 comparedb_tables_read 역할 부여
GRANT comparedb_tables_read TO readonly_user;

-- readonly_user의 기본 역할 설정
ALTER USER readonly_user DEFAULT ROLE ALL;

-- readonly_user 사용자가 COMPAREDB 스키마의 테이블을 스키마 명 없이 조회할 수 있도록 동의어 생성
BEGIN
    FOR t IN (SELECT table_name FROM all_tables WHERE owner = 'COMPAREDB') LOOP
        EXECUTE IMMEDIATE 'CREATE SYNONYM readonly_user.' || t.table_name || ' FOR COMPAREDB.' || t.table_name;
    END LOOP;
END;
/
...
-- 수동으로 통계 수집
EXEC DBMS_STATS.GATHER_SCHEMA_STATS('COMPAREDB');
