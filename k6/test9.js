...
// 환경 설정
const CONFIG = {
    ENDPOINT: __ENV.API_ENDPOINT || 'https://allcomparedb-latest.onrender.com/api/database/execute', // https://allcomparedb-latest.onrender.com/api/database/execute http://192.168.0.71:8080/api/database/execute
   // DB_TYPE: __ENV.DB_TYPE || 'ORACLE',   // ORACLE POSTGRES MARIADB
    START_VUS: __ENV.START_VUS || 10,      // 시작 사용자 수
    TARGET_VUS: __ENV.TARGET_VUS || 30,   // 목표 사용자 수
    DURATION: __ENV.DURATION || '600s'      // 각 단계별 지속 시간
};

// 사용 가능한 DB 타입 배열
const DB_TYPES = ['ORACLE', 'POSTGRES', 'MARIADB']; // 'ORACLE', 'POSTGRES', 'MARIADB'
...
function randomFloatBetween(min, max) {
    ...
    return Math.round(randomNum * 100) / 100;  // 100을 곱한 후 반올림, 다시 100으로 나누어 소수점 2자리로
}
...
export let options = {
    stages: [
        ...
        { duration: '60s', target: CONFIG.TARGET_VUS }, // 목표 부하까지 증가
...
export default function () {
    const randomCustomerId = randomIntBetween(1, 32); //32
    const randomProductId = randomIntBetween(1, 34); //34
    const randomOrderId = randomIntBetween(1, 30); //30
    ...
    const selectedDbType = DB_TYPES[randomIntBetween(0, DB_TYPES.length - 1)];

    const response = http.post(
        `${CONFIG.ENDPOINT}?dbType=${selectedDbType}`,
        query,
        REQUEST_PARAMS
    );
    ...
    if (!passed) {
//	console.error(`Received unexpected status code: ${response.status}`,`Failed Query: ${selectedDbType} / ${query}`);
	console.error(
    		`Received unexpected status code: ${response.status}`,
		`Failed Query: ${selectedDbType} / ${query}`,
		`Request URL: ${CONFIG.ENDPOINT}?dbType=${selectedDbType}`,
		`Request Headers: ${JSON.stringify(REQUEST_PARAMS.headers)}`,
