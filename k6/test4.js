...
const CONFIG = {
    ...
    TARGET_VUS: __ENV.TARGET_VUS || 100,   // 목표 사용자 수
...
export let options = {
    ...
    thresholds: {
        http_req_duration: ['p(95)<500'],  // 응답시간 임계값
...
const DB_TYPES = ['POSTGRES', 'ORACLE', 'MARIADB'];

// Function to handle requests for a specific DB type
function sendToDatabase(dbType) {
    // 랜덤 값 생성
    ...
    const randomOrderDetailProductId = randomIntBetween(1, 35);
    ...
    check(response, {
        ...
        'is response time < 500ms': (r) => r.timings.duration < 500, // 변경된 응답 시간 체크
    });
}

// Default function to execute requests for all DB types
export default function () {
    DB_TYPES.forEach((dbType) => {
        sendToDatabase(dbType); // 각 DB에 대한 요청 함수 실행
    });
    sleep(1); // 짧은 대기
}
